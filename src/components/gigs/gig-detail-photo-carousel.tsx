...
export function GigDetailPhotoCarousel({
  media,
  gigTitle,
}: GigDetailPhotoCarouselProps) {
  ...
  const galleryPhotos = media
    ...
    .sort((a, b) => a.sortOrder - b.sortOrder);
  const photosToShow =
    featuredPhotos.length > 0 ? featuredPhotos : galleryPhotos;
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        <CarouselContent className="-ml-2 md:-ml-4">
          {photosToShow.map((photo) => (
            <CarouselItem key={photo.id} className="pl-2 md:pl-4">
              <div className="hover:border-accent-muted/50 relative aspect-video overflow-hidden rounded-none border-2 border-white/10 bg-black/20 transition-all hover:shadow-[0_0_15px_var(--accent-muted)]">
                <Image
        ...
        {photosToShow.length > 1 && (
          <>
            <CarouselPrevious className="hover:border-accent-muted absolute top-1/2 left-2 h-8 w-8 -translate-y-1/2 rounded-none border-2 border-white/20 bg-black/60 text-white backdrop-blur-sm hover:bg-black/80" />
            <CarouselNext className="hover:border-accent-muted absolute top-1/2 right-2 h-8 w-8 -translate-y-1/2 rounded-none border-2 border-white/20 bg-black/60 text-white backdrop-blur-sm hover:bg-black/80" />
          </>
        )}
      </Carousel>
    </div>
