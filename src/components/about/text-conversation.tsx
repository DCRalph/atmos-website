...
function getBubbleRadii(index: number, visibleMessages: Message[]) {
...
function getGapAbove(index: number, visibleMessages: Message[]) {
  if (index === 0) return 0;
  ...
  return (
    ...
    >
      <div
        className={`px-4 py-3 ${
          side === "right"
            ? "rounded-2xl rounded-br-[5px] bg-blue-600"
          ...
          {[0, 1, 2].map((i) => (
            <motion.div
              ...
              className={`h-[6px] w-[6px] rounded-full ${
                side === "right" ? "bg-white/60" : "bg-muted-foreground/50"
              }`}
              animate={{
                y: [0, -4, 0],
                opacity: [0.3, 1, 0.3],
...
function MessageBubble({
  ...
  return (
    ...
    >
      <motion.div
        className={`max-w-[80%] px-3 py-[6px] text-[14px] leading-[19px] md:max-w-[78%] md:px-3.5 md:py-[7px] md:text-[15px] md:leading-[20px] ${
          isRight
...
export function TextConversation() {
  ...
  useEffect(() => {
    const unsubscribe = scrollYProgress.on("change", (progress) => {
      ...
      const msgProgress = Math.max(0, Math.min(1, (progress - 0.12) / 0.66));

      // Compute how many full messages to show
      const fractional = msgProgress * (messages.length + 1);
      const targetCount = Math.min(Math.floor(fractional), messages.length);
      ...
      const shouldType = targetCount < messages.length && remainder > 0.35;
    ...
    >
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center px-4 py-6 md:px-6 md:py-0">
        ...
        <div className="w-full max-w-sm">
          <div className="border-border bg-card rounded-4xl border p-1.5 shadow-2xl shadow-black/50 md:rounded-[2.5rem] md:p-2">
            {/* Status bar */}
            <div className="flex items-center justify-center pt-2 pb-0.5 md:pt-3 md:pb-1">
              <div className="bg-muted h-[5px] w-[80px] rounded-full" />
            </div>

            {/* Chat header */}
            <div className="flex items-center gap-2.5 px-3 py-2 md:gap-3 md:px-4 md:py-2.5">
              <div className="bg-muted flex h-8 w-8 items-center justify-center rounded-full md:h-9 md:w-9">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-[11px]">group chat</p>
              </div>
            </div>
            ...
            {/* Messages area */}
            <div className="bg-background flex max-h-[300px] min-h-[300px] flex-col justify-end overflow-hidden rounded-b-3xl px-2.5 py-2.5 md:max-h-[420px] md:min-h-[420px] md:rounded-b-4xl md:px-3 md:py-3">
              <AnimatePresence initial={false} mode="popLayout">
                ...
                {showTyping && <TypingBubble key="typing" side={typingSide} />}
              </AnimatePresence>
            </div>

            {/* Input bar */}
            <div className="flex items-center gap-2 px-3 pt-1.5 pb-3 md:pt-2 md:pb-4">
              <div className="border-border/60 bg-secondary/50 flex h-[34px] flex-1 items-center rounded-full border px-4 md:h-[36px]">
